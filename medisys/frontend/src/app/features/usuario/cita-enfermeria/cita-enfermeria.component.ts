...
@Component({
  selector: 'app-cita-enfermeria',
  standalone: true,
  imports: [FormsModule, RouterLink],
  template: `
    <div class="cita-container">
      <h2>Cita de Enfermería</h2>
      ...
      @if (token()) {
        ...
        <div class="form-card">
          <div class="input-group">
            <label for="motivoEnf">Motivo</label>
            <textarea id="motivoEnf" [(ngModel)]="motivo" placeholder="Describa su motivo (opcional)" rows="3" class="form-control-lg"></textarea>
          </div>

          <div class="input-group">
            <label for="receta">Receta médica (opcional)</label>
            <input id="receta" type="file" (change)="onFileChange($event)" class="form-control-lg" />
          </div>

          <button (click)="reservar()" class="btn-primary">
...
export class CitaEnfermeriaComponent implements OnInit, OnDestroy {
  motivo = ''
  token = signal('')
  error = signal('')
  file: File | undefined
  ...
  onFileChange(event: any) { this.file = event.target.files[0] }

  reservar() {
    this.svc.crearCitaEnfermeria(this.motivo, this.file).subscribe({
      next: (res: any) => {
        if (res.tokenPago) {
  ...
  async generarQR(token: string) {
    ...
    if (canvas) {
      const url = `http://192.168.1.14:4200/usuario/pagar/${token}`
  ...
  iniciarPolling(token: string) {
    this.pollingInterval = setInterval(() => {
      this.svc.getEstadoPorTokenEnfermeria(token).subscribe({
        next: (cita: any) => {
